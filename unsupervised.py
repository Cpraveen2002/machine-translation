...
def get_tokens(input_string):
	stripped_text = input_string.strip().split(' ')
	tokens = []
...
def get_unique_words(input_string):
	stripped_string = input_string.strip().split(' ')
	words_set = set()
	words = []
	for word in stripped_string:
...
def get_frequent_words(input_string, n):
	chop_off_distance = 10
	word_list = {}
	...
	word_list = sorted(list(word_list.items()), key=lambda item: item[1], reverse = True)

	top_n_words = []
	for x in range(len(word_list)):
		top_n_words.append(word_list[x][0])
	# for x in range(n):
...
def main(language, input_text):
	
	textfiles = ["./data/hindi.txt", "./data/telugu.txt"]
	# languages = ["hindi", "telugu"]
	# language = ""
	# file_path = textfiles[int(sys.argv[1])]
	if language == "telugu":
		file_path = textfiles[1]
		# language = languages[1]
	elif language == "hindi":
		file_path = textfiles[0]
		# language = languages[0]
	
	number_of_top_words = 100
	tokens = get_tokens(input_text)
	word_types = get_unique_words(input_text)
	top_n_words = get_frequent_words(input_text, number_of_top_words)
	# print(tokens)
	# print(word_types)
	# print(top_n_words)
	...
	# selected_word_index = [1, 94, 109, 44, 77, 131, 156, 206, 144, 99, 14, 29, 222, 172, 157, 62, 55, 181, 100, 79, 27, 42, 19, 40, 140, 169]

	centroids = []
	for x in range(number_of_clusters):
		# centroids.append(feature_vectors[selected_word_index[x]])
		if selected_word_index[x] < len(feature_vectors):
			centroids.append(feature_vectors[selected_word_index[x]])
	...
	file_output = codecs.open("./output/"+ language +"_clusters.txt", 'w', 'utf-8')
	...
	print("Kindly check ./output/"+ language +"_clusters.txt for clusters and words in that cluster\n")

	file_output = codecs.open("./data/"+language+"_training_unsupervised.txt", 'w', 'utf-8')
	file_pointer = codecs.open(file_path, 'r', encoding='utf-8')
	file_contents = file_pointer.readlines()
	for x in range(len(file_contents)):
		line = file_contents.pop(0).strip().split(' ')
	...
	returned_string = helper.main(language, input_text)
